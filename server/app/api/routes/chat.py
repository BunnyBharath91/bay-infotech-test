...
async def chat(
    ...
    if settings.USE_LLM:
        # Use LLM to rephrase KB content
        ...
        # KB-only mode: Return extracted KB content directly
        logger.info("USE_LLM=false: Returning KB-based response without LLM")
        
        # Extract and format KB content
        kb_text_parts = []
        for chunk in kb_chunks[:3]:  # Top 3 most relevant chunks
            kb_text_parts.append(chunk.chunk_text.strip())
        
        # Build a simple template-based response
        if kb_text_parts:
