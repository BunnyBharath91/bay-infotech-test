    ...
    try:
        ...
        try:
            escalation_result = await db.execute(
                ...
                .where(AnalyticsEvent.meta_data['escalated'].astext == 'true')
            )
            escalation_count = escalation_result.scalar() or 0
        except Exception as e:
            logger.warning(f"Error counting escalations: {e}")
            escalation_count = 0
        ...
        logger.info(f"Metrics summary generated for last {hours} hours")
        return result
    except Exception as e:
        logger.error(f"Error in get_metrics_summary: {type(e).__name__}: {e}", exc_info=True)
        raise
